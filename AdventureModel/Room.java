...
public class Room implements Serializable {
    ...
    public String getObjectString() {
        ...
        String tempDescrip = "";
        //loops through description
        int counter = 0;
        while (counter < objectsInRoom.size()) {
            String descripy = objectsInRoom.get(counter).getDescription();
            descrip.append(descripy);
            if (counter != objectsInRoom.size() - 1) {
                descrip.append(", ");
            }
            else {
                tempDescrip += "temp";
    ...
    public String getCommands() {
        String stringy = "";
        //go through motionTable
        Iterator<Passage> iterator = this.motionTable.getDirection().iterator();
        while (iterator.hasNext()) {
            Passage currp = iterator.next();
            stringy += currp.getDirection() + ", ";
        }
        //get rid of the last comma
        String stringyAgain = stringy.substring(0, stringy.length() - 2);
        return stringyAgain;
    }
